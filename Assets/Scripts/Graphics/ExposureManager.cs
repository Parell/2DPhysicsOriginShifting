...
namespace Decel
{
    public class ExposureManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            Vector2 camPos = localCamera.transform.position;
            ...
            // {
            //     if (bodies[i].bodyData.type == BodyData.BodyType.Kinematic)
            //     {
            ...
            //         if (bodySize > 1.0f)
            //         {
            //             Vector2 bodyPosition = bodies[i].rb.position;
            //             Vector2 targetVectorToSun = starBody.rb.position - bodyPosition;
            //             Vector2 targetVectorToCam = camPos - bodyPosition;

            //             float targetRelAngle = (float)Vector2.Angle(targetVectorToSun, targetVectorToCam);
            ...
            //             float CBAngle = Mathf.Max(0.0f, Vector2.Angle((bodyPosition - camPos).normalized, camAngle) - bodySize);
